...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        # Sigmoid activation function, for one endpoint output unit
        def f_sigmoid(x): return 1.0 / (1.0 + np.exp(-x))

        def f_softmax(x): return np.exp(x) /\
            np.tile(np.exp(x).sum(axis=1), (num_classes, 1)).T  # Softmax

        if num_classes > 1:
            y_pred = f_softmax(scores)  # Softmax classifier
        else:
            y_pred = f_sigmoid(scores)  # Sigmoid activation output
        # print(f"X: {X}")
        # print(f"y_pred: {y_pred}")
        # input()
        y_onehot = np.eye(num_classes)[
            y] if num_classes > 1 else np.expand_dims(y, 1)

        # loss = np.square(y_pred - y_onehot).sum() # MSE loss
        # Softmax cross entropy loss
        # categorical class mode
        loss = - y_onehot * \
            np.log(np.maximum(np.finfo(y_pred.dtype).eps, y_pred))

        if num_classes == 1:  # binary class mode
            loss += - (1-y_onehot) * \
                np.log(np.maximum(np.finfo(y_pred.dtype).eps, (1-y_pred)))
        ...
        # print(f"grad_a2: {grad_a2}")
        # input()
        grads['W2'] = h1.T.dot(grad_a2)
        # print(f"grads['W2']: {grads['W2']}")
        # input()
        ...
        for it in tqdm(range(num_iters)):
            ...
            if it % iterations_per_epoch == 0:
                ...
                train_acc = (self.predict(
                    X_batch, num_classes=1, output_label=True) == y_batch).mean()
                val_acc = (self.predict(
                    X_val, num_classes=1, output_label=True) == y_val).mean()
    ...
    def predict(self, X, num_classes=10, output_label=False):
        """
        ...
        if num_classes > 1:
            # Multi-classes
            logits = f_softmax(scores)  # Softmax classifier
        else:
            logits = f_sigmoid(scores)  # Sigmoid activation output
